...
class CaretakerViewSetTest(TestCase):
    ...
    def setUp(self):
        PlanFeatureLimit.objects.get_or_create(
            plan=self.fp, feature_key="max_caretakers", defaults={"value": "2"}
        )
        PlanFeatureLimit.objects.get_or_create(
            plan=self.pp, feature_key="max_caretakers", defaults={"value": "10"}
        )
    ...
    def test_unauth_list(self):
        self.assertEqual(self.client.get("/properties/caretakers/").status_code, 401)

    def test_list(self):
        self.assertEqual(self._auth().get("/properties/caretakers/").status_code, 200)

    def test_create_bad(self):
        self.assertEqual(
            self._auth().post("/properties/caretakers/", {}).status_code, 400
        )
