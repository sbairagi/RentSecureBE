...
class RentRecordViewSetTest(TestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.o = User.objects.create_user(username='rro@t.com', email='rro@t.com', password='p', full_name='RRO', phone='+1')
    ...
    def _auth(self): c = APIClient(); c.credentials(HTTP_AUTHORIZATION=f'Bearer {RefreshToken.for_user(self.o).access_token}'); return c
    def test_unauth_list(self): self.assertEqual(self.client.get('/properties/rent-records/').status_code, 401)
    def test_list(self): self.assertEqual(self._auth().get('/properties/rent-records/').status_code, 200)

class ExtraChargeViewSetTest(TestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.o = User.objects.create_user(username='eco@t.com', email='eco@t.com', password='p', full_name='ECO', phone='+1')
    ...
    def _auth(self): c = APIClient(); c.credentials(HTTP_AUTHORIZATION=f'Bearer {RefreshToken.for_user(self.o).access_token}'); return c
    def test_extra_charge_unauth(self): self.assertEqual(self.client.post('/properties/extra-charges/', {}).status_code, 401)