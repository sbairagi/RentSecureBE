...
These tests live in ``properties/tests`` because ``rentsecure_be`` is the
bottom layer in the import-linter layered contract and cannot import
``core.models`` or ``properties.models`` at the top level.  ``properties``
has no layered contract, so it can freely import both packages.
"""
...
# ``process_rent_payout`` references ``RentRecord`` only under ``TYPE_CHECKING``
# but performs an ``isinstance`` check at runtime.  Because ``rentsecure_be``
...
class ProcessRentPayoutTest(TestCase):
    ...
    def test_process_payout_success(self):
        ...
        with patch(
            ...
            mock_payout.return_value = {"status": "SUCCESS"}
            with patch(
                "notification.services.rent_notify_service.notify_owner_post_payout"
            ):
                with patch(
