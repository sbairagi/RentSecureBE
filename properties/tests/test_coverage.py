...
class ExtraChargeModelTest(TestCase):
    def test_create_and_str(self):
        ...
        ec = ExtraCharge.objects.create(
            unit=u, renter=r, name="Water", amount=Decimal("50"), due_date=date.today()
        )
...
class RentAgreementDraftCreateTest(TestCase):
    def test_create(self):
        ...
        rad = RentAgreementDraft.objects.create(
            user=o, renter=r, unit=u, file="draft.pdf"
        )
...
class FeatureEnforcerCoverageTest(TestCase):
    def setUp(self):
        ...
        self.fp, _ = SubscriptionPlan.objects.get_or_create(
            name="fec_free",
            defaults={
        ...
        self.pp, _ = SubscriptionPlan.objects.get_or_create(
            name="fec_pro",
            defaults={
        ...
        PlanFeatureLimit.objects.get_or_create(
            plan=self.fp, feature_key="max_buildings", defaults={"value": "2"}
        )
    ...
    def test_feature_enforcer_active_sub(self):
        UserSubscription.objects.update_or_create(
            user=self.user, defaults={"plan": self.pp, "is_active": True}
        )
        ef = FeatureEnforcer(user=self.user)
...
class CoreModelCoverageTest(TestCase):
    ...
    def test_usage_limit_str(self):
        ...
        ul = UsageLimit.objects.create(
            user=user, feature_key="max_units", usage_count=3
        )
