...
from core.models import SubscriptionPlan, PlanFeatureLimit, UserSubscription, AddOnPurchase, UsageLimit
...
class FeatureEnforcerEdgeCases(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='fee_u1', email='feeu1@test.com', password='p', full_name='FEE1', phone='+1')
        ...
        self.free_plan = SubscriptionPlan.objects.get_or_create(
            name='free', defaults={'monthly_price': Decimal('0'), 'yearly_price': Decimal('0'), 'features': 'Free', 'is_active': True}
        )[0]
    ...
    def test_get_active_limit_expired_no_grace(self):
        pro = SubscriptionPlan.objects.create(name='pro', monthly_price=Decimal('499'), yearly_price=Decimal('4999'), features='Pro', is_active=True)
        PlanFeatureLimit.objects.create(plan=pro, feature_key='max_buildings', value='20')
        self._create_sub(pro, end_date=timezone.now() - timedelta(days=10))
        enforcer = FeatureEnforcer(self.user)
        self.assertEqual(enforcer.get_active_limit('max_buildings'), 2)

    def test_get_active_limit_within_grace(self):
        pro = SubscriptionPlan.objects.create(name='pro2', monthly_price=Decimal('499'), yearly_price=Decimal('4999'), features='Pro2', is_active=True)
        PlanFeatureLimit.objects.create(plan=pro, feature_key='max_buildings', value='20')
    ...
    def test_can_create_at_limit(self):
        UsageLimit.objects.create(user=self.user, feature_key='max_buildings', usage_count=2)
        enforcer = FeatureEnforcer(self.user)
    ...
    def test_addon_extends_limit(self):
        pro = SubscriptionPlan.objects.create(name='pro3', monthly_price=Decimal('499'), yearly_price=Decimal('4999'), features='Pro3', is_active=True)
        PlanFeatureLimit.objects.create(plan=pro, feature_key='max_buildings', value='10')
    ...
    def test_plan_with_subscription_but_no_limit_returns_zero(self):
        pro = SubscriptionPlan.objects.create(name='pro_limitless', monthly_price=Decimal('0'), yearly_price=Decimal('0'), features='Pro', is_active=True)
        self._create_sub(pro)
        enforcer = FeatureEnforcer(self.user)
    ...
    def test_is_expired_with_subscription_no_end_date(self):
        pro = SubscriptionPlan.objects.create(name='pro4', monthly_price=Decimal('0'), yearly_price=Decimal('0'), features='Pro4', is_active=True)
    ...
    def test_is_expired_with_subscription_future_end(self):
        pro = SubscriptionPlan.objects.create(name='pro5', monthly_price=Decimal('0'), yearly_price=Decimal('0'), features='Pro5', is_active=True)
        self._create_sub(pro, end_date=timezone.now() + timedelta(days=30))
        enforcer = FeatureEnforcer(self.user)
        self.assertFalse(enforcer.is_expired())

    def test_is_expired_with_subscription_past_end(self):
        pro = SubscriptionPlan.objects.create(name='pro6', monthly_price=Decimal('0'), yearly_price=Decimal('0'), features='Pro6', is_active=True)
        self._create_sub(pro, end_date=timezone.now() - timedelta(days=1))
        enforcer = FeatureEnforcer(self.user)
        self.assertTrue(enforcer.is_expired())

    def test_is_past_grace_period_with_subscription_no_end(self):
        pro = SubscriptionPlan.objects.create(name='pro7', monthly_price=Decimal('0'), yearly_price=Decimal('0'), features='Pro7', is_active=True)
        self._create_sub(pro, end_date=None)
        enforcer = FeatureEnforcer(self.user)
