...
from core.models import NotificationPreference, SubscriptionPlan, UserProfile, UserSubscription
...
class UserSignalTest(TestCase):
    def test_user_creation_creates_profile(self):
        user = User.objects.create_user(username='sig1@t.com', email='sig1@t.com', password='p', full_name='Sig User', phone='+1')
        self.assertTrue(hasattr(user, 'userprofile'))
        self.assertIsInstance(user.userprofile, UserProfile)

    def test_user_creation_creates_notification_preference(self):
        user = User.objects.create_user(username='sig2@t.com', email='sig2@t.com', password='p', full_name='Sig User 2', phone='+2')
    ...
    def test_user_creation_assigns_free_plan(self):
        user = User.objects.create_user(username='sig3@t.com', email='sig3@t.com', password='p', full_name='Sig User 3', phone='+3')
        self.assertTrue(hasattr(user, 'usersubscription'))
        self.assertEqual(user.usersubscription.plan.name, 'free')

    def test_user_profile_defaults(self):
        user = User.objects.create_user(username='sig4@t.com', email='sig4@t.com', password='p', full_name='Sig User 4', phone='+4')
    ...
    def test_notification_preference_defaults(self):
        user = User.objects.create_user(username='sig5@t.com', email='sig5@t.com', password='p', full_name='Sig User 5', phone='+5')
