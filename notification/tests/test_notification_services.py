...
from notification.services.whatsapp_service import (
    send_whatsapp_message, send_whatsapp_audio,
    upload_to_s3, send_agreement_via_whatsapp
)
from notification.services.voice_service import generate_voice_note
from notification.services.rent_notify_service import (
    notify_renter, notify_owner, send_payout_notification,
    notify_owner_post_payout
)
from notification.services.extra_charge_reminders import send_due_extra_charge_reminders
from notification.services.late_fees_notify_service import notify_renter_about_late_fee, notify_owner_about_late_fee
from notification.utils import send_push_notification, send_whatsapp_message as utils_send_whatsapp
...
class WhatsAppServiceTest(TestCase):
    ...
    def test_send_agreement_via_whatsapp(self):
        ...
        with patch('notification.services.whatsapp_service.send_whatsapp_message') as mock_send:
...
class RentNotifyServiceTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='notify_user', email='notify@test.com',
            password='p', full_name='Notify', phone='+1'
        )
...
class NotificationUtilsTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='notif_util', email='nu@test.com',
            password='p', full_name='NU', phone='+1'
        )
    ...
    def test_send_whatsapp(self, mock_client):
        with override_settings(TWILIO_ACCOUNT_SID='test_sid', TWILIO_AUTH_TOKEN='test_token', TWILIO_WHATSAPP_NUMBER='whatsapp:+14155238886'):
