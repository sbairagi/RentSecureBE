...
class PayOwnerAfterRentTest(TestCase):
    def setUp(self):
        ...
        self.building = Unit.objects.create(
            owner=self.owner,
            unit="101",
        ...
        self.renter_model = Renter.objects.create(
            unit=self.building.unit_set.first(),
            name="Test Renter",
            phone="+911234567890",
            email="tr@test.com",
...
class ProcessRentPayoutTest(TestCase):
    ...
    def test_process_payout_success(self):
        with patch(
            ...
            mock_payout.return_value = {"status": "SUCCESS"}
            with patch(
                "rentsecure_be.services.cashfree_service.notify_owner_post_payout"
            ):
                with patch(
                    "rentsecure_be.services.cashfree_service.send_payout_notification"
                ):
