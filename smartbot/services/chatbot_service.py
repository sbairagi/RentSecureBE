...
def handle_chat_message(user, message):
    ...
    if "rent due" in message:
        next_due = (
            RentRecord.objects.filter(renter__user=user, paid=False)
            .order_by("due_date")
            .first()
        )
        if next_due:
    ...
    if "agreement" in message:
        ...
        return (
            f"📄 Here is your latest rent agreement: {latest.agreement_pdf.url}"
            if latest and latest.agreement_pdf
            else "No agreement found."
        )
